...
`r fa("location-dot", fill = "black")` [<span style="color: black">**Department of Geography**</span>](https://www.geog.ucl.ac.uk/) .my-style[&] [<span style="color: black">**Center for Advanced Spatial Analysis**</span>](https://www.ucl.ac.uk/bartlett/casa/) .my-style[UCL]
...
- ↑ uQOL → attract more global opportunities, investment → ↑ higher city economic values
]]
---
class: left
background-size: cover
...
- It is important to determine the weight of each index, The function is below:


$$I = W_1*NDVI + W_2*NDWI + W_3*NDBI + {...}$$


- Based on this composite index, we can generate a cluster map, the input data is index for each sampled point, here, a number of clusters are obtained, with the smallest possible distance within clusters and the largest possible distance between clusters, so that land cover types of land can be distinguished clearly.

- Here is an output example of this model:

]

.pull-right[

Cluster a: urban area, Cluster b: water, 
Cluster c: bare earth

  <img src="images/cluster_example.png" height="250px"/>
  Cluster_example
